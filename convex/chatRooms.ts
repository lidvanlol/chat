...
export const getChatRoom = query({
  handler: async (ctx, { chatRoomId }: { chatRoomId: Id<'chatRooms'> }) => {
    return await ctx.db.get(chatRoomId);
  },
});
...
export const joinChatRoom = mutation({
  ...
  handler: async (ctx, args) => {
    const existingMembership = await ctx.db
      ...
      .filter(q =>
        q.and(q.eq(q.field('chatRoomId'), args.chatRoomId), q.eq(q.field('userId'), args.userId)),
      )
      .first();
